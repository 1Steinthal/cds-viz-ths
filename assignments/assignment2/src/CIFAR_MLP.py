...
def main():
    ...
    #X_test_p = X_preproces(X_test, 32, "test") #Sparing preprocessing time

    inpath = os.path.join("..", "data")
    X_train_p = X_load(os.path.join(inpath, "X_train_preprocessed.npy")) #... therefore we load already processed data
...
def X_preproces(data, dim, name): 
    output_array = np.empty((1, dim*dim)) #Either because of this definition

    for i, img in enumerate(data):
        img_g = cv2.cvtColor(data[i], cv2.COLOR_BGR2GRAY) 
        img_g = img_g.reshape(-1, dim*dim) #or here... (I've just taken your code, Ross, but can't figure out what -1 does)
        img_g = img_g/255.0
        output_array = np.append(output_array, img_g, axis=0) 
...
#Loading in already preprocessed data
def X_load(filename):
...
def save_cls_reports_and_plots(model, X_test, y_test, lbl_names, path_spec, buf):
    ...
    #Loss curve
    if buf == 1: #Only available for MLP
